...
const LeadershipCard: FC<{ item: LeadershipItem; index: number }> = ({ item, index }) => {
  ...
  const inner = (
    <>
      <div className="flex items-start justify-between gap-2 mb-1">
        ...
        {item.linkUrl && <ArrowUpRight className="w-3 h-3 text-[var(--text-faint)] shrink-0" />}
      </div>
      <h3 className="text-sm font-semibold text-[var(--text)] mb-1">{item.title}</h3>
      <p className="text-[11px] text-[var(--text-muted)] leading-relaxed mb-3">{item.description}</p>
      {item.photos && item.photos.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-3">
          {item.photos.map((src, pi) => (
            <div key={pi} className="relative w-16 h-20 rounded-lg overflow-hidden border border-[var(--border)]">
              <img
                src={src}
  ...
  const baseClass = "block border-l-2 border-[var(--border-strong)] pl-4 mt-4 first:mt-0";
...
export const Leadership = forwardRef<HTMLElement>((props, ref) => {
  return (
    <section ref={ref} className="h-full bg-[var(--surface)] border border-[var(--border)] rounded-3xl p-6 lg:p-8 flex flex-col justify-center">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          <h2 className="text-sm font-mono text-[var(--text-subtle)] uppercase mb-6 tracking-widest">Leadership</h2>
        ...
        <div className="flex flex-col">
          <h2 className="text-sm font-mono text-[var(--text-subtle)] uppercase mb-6 tracking-widest">Certifications</h2>
          <div className="flex flex-col gap-3">
            {PORTFOLIO_DATA.certifications.slice(0, 3).map((cert, index) => (
              <motion.a
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.1, duration: 0.5 }}
                className="group flex flex-col p-4 rounded-2xl bg-[var(--surface-2)] border border-[var(--border)] hover:border-[var(--border-strong)] transition-colors relative"
              >
                <ArrowUpRight className="w-4 h-4 text-[var(--text-faint)] group-hover:text-[var(--accent)] transition-colors absolute top-4 right-4" />
                <span className="text-[var(--accent)] font-bold text-[10px] uppercase tracking-widest mb-2 opacity-50 block">
                  Verified {cert.year}
                </span>
                <span className="text-[var(--text)] text-xs font-semibold pr-6">{cert.name}</span>
              </motion.a>
            ))}
          </div>
        </div>
